...
$cache_folder = sanitize_text_field($_POST['cache-folder']);
...
$exclude_pages = array_map('intval', $_POST['pages'] ?? array());
$exclude_pages = maybe_serialize($exclude_pages) ?? array();
...
$result = false;
	if ( file_exists( untrailingslashit($cache_folder) .DIRECTORY_SEPARATOR. sanitize_text_field($_GET['delete-specific-cache']).".cache" ) ) {
		unlink( untrailingslashit($cache_folder) .DIRECTORY_SEPARATOR.sanitize_text_field( $_GET['delete-specific-cache']).".cache" );
		$result = true;
	}
...
&& ( wp_verify_nonce( $nonce, 'easy-cache-nonce' )) ) {
//Clean unnecessary slashes
$cache_folder = sanitize_text_field(get_option('easy_cache_option_cache_folder'));
$result = false;
...
<div id="message" class="updated error">
  ...
  <form name="form-cache" id="form-cache" action="?page=<?php echo sanitize_url($_GET['page']); ?>&t=<?php echo time(); ?>" method="post">
...
<?php _e("You can delete associated with given post/page cached file by clicking on the red X","bodi0-easy-cache"); ?>.
        ...
        <?php 
				...
				foreach ( $pages as $page ) {
					...
					echo '( <a href="?page='. sanitize_url($_GET['page']) .'&delete-specific-cache='.sha1(get_permalink($page->ID)).
       ...
       <strong><?php _e("Posts","bodi0-easy-cache"); ?></strong>
        <div style="height: auto;max-height: 400px;min-height: 40px;overflow: scroll;width: 400px;">
				...
				foreach ( $posts as $post ) {
					...
					echo '( <a href="?page='. sanitize_url($_GET['page']) .'&delete-specific-cache='.sha1(get_permalink($post->ID)).
					'&amp;_wpnonce='. wp_create_nonce( 'easy-cache-nonce' ) 
					.'" title="'.__("Delete associated cached file","bodi0-easy-cache").'"
					class="delete"
					>X</a> ) ';}
	...
	?></td>
      ...
      <tr class="alternate">
        <td colspan="2" style="vertical-align:top"><p>
            ...
            &nbsp;<a accesskey="d" href="?page=<?php echo sanitize_url($_GET['page']); ?>&amp;restore=defaults&amp;_wpnonce=<?php echo wp_create_nonce( 'easy-cache-nonce' ) ?>" class="button-secondary cancel"><?php _e("Restore default settings", "bodi0-easy-cache"); ?></a> 
          </p></td>
      </tr></tbody>
    </table>
    <?php 
...
?>  
<p> <a href="?page=<?php echo sanitize_url($_GET['page'])?>&amp;_wpnonce=<?php echo wp_create_nonce( 'easy-cache-nonce' ) ?>" class="alignright" >
...
?></strong>
    ...
    <tr>
      <td colspan="2">
   &nbsp;<a accesskey="c" href="?page=<?php echo sanitize_url($_GET['page']); ?>&amp;delete-cache=reset&amp;_wpnonce=<?php echo wp_create_nonce( 'easy-cache-nonce' ) ?>" class="button-secondary cancel"><?php _e("Delete all cached files", "bodi0-easy-cache"); ?></a> 

<?php if ($css_file_exists == 1) { ?>
   &nbsp;<a accesskey="s" href="?page=<?php echo sanitize_url($_GET['page']); ?>&amp;delete-css=reset&amp;_wpnonce=<?php echo wp_create_nonce( 'easy-cache-nonce' ) ?>" class="button-secondary cancel"><?php _e("Delete combined and minified CSS file", "bodi0-easy-cache"); ?></a> 
